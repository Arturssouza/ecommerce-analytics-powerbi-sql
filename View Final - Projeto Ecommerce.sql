...
SELECT
    ...
    o.customer_id, 
    i.seller_id, 
    STR_TO_DATE(o.order_purchase_timestamp, '%Y-%m-%d %H:%i:%s') AS data_pedido, -- Aqui foi ajustada a Coluna para Data de Pedido
    ...
    END AS categoria, -- Aqui resolvemos o problema de algumas categorias que vieram em branco
    DATEDIFF(
        STR_TO_DATE(o.order_delivered_customer_date, '%Y-%m-%d %H:%i:%s'), 
        STR_TO_DATE(o.order_estimated_delivery_date, '%Y-%m-%d %H:%i:%s')
    ) AS dias_de_atraso -- Aqui fizemos a diferença entre data de entrega e data estimada de entrega para calcular a performance logística
